...
gsea2_combined <- function(mexp=NULL, pheno1.col=NULL, pheno2.col=NULL, reflist=NULL, gspos, gsneg, nullDist, w=1, sizelim=1, max=F, abs=F, pval=F) {
  ...
  # check input format
  if size(rk,2)>size(rk,1), rk = t(rk) 
  if size(rs,2)>size(rs,1), rs = t(rk)
  nes = NA
  pv  = NA
